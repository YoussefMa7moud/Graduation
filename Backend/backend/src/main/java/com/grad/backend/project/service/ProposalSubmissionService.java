...
public NdaPartiesResponse getNdaParties(Long submissionId, Long currentUserId) {
    ...
    if (s.getClientType() == ClientType.COMPANY) {
        ClientCompany cc = clientCompanyRepo.findById(cl.getId()).orElseThrow(() -> new RuntimeException("Client company not found"));
        ...
        ClientPerson cp = clientPersonRepo.findById(cl.getId()).orElseThrow(() -> new RuntimeException("Client person not found"));
        partyB = NdaPartiesDTO.builder()
...
public String verifyActor(Long submissionId, Long userId) {
    ...
    if (userId.equals(s.getClient().getId())) return "client";
    if (userId.equals(s.getSoftwareCompany().getId())) return "company";
    return "none";
}
}