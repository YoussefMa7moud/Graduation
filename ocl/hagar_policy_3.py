...
# --- Policy Info ---
# Policy ID: 3
# Description: working hours must not exceed 8 hours

# --- Context Class ---
basemodel_class = Class(name="BaseModel")

# --- Add dynamic properties ---
workingHours_prop = Property(name="workingHours", type=StringType)
basemodel_class.attributes = {workingHours_prop}
...
POLICY_CONSTRAINT = Constraint(
    ...
    expression='context BaseModel inv: self.workingHours %3C%3D 8',
    language="OCL"
)
...
dynamic_obj.workingHours = value
print(f"Set workingHours = {value} (from test description)")

context_om = ObjectModel(name="BaseModelModel", objects={dynamic_obj})


# --- Evaluate Policy Constraint ---
print(f"\nTesting Policy #3: 'working hours must not exceed 8 hours'")
...
if EVAL_MODE == "OCL":
    ...
    expr = "self.workingHours %3C%3D 8".strip()
