...
import com.grad.backend.project.service.ProjectTechDocValidationService;
...
public class CompanyProjectController {
    ...
    private final ProjectTechDocValidationService projectTechDocValidationService;
    ...
    @PostMapping("/{id}/validate-tech-doc")
    public ResponseEntity<?> validateTechDoc(
            @PathVariable Long id,
            @AuthenticationPrincipal User user,
            @RequestBody TechDocValidationRequest request) {
        if (user == null) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                    .body(Map.of("error", "Not authenticated"));
        }
