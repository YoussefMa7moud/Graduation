...
# --- Policy Info ---
# Policy ID: 1
# Description: minimum delivery date for a software is 90 days

# --- Context Class ---
basemodel_class = Class(name="BaseModel")

# --- Add dynamic properties ---
deliveryDate_prop = Property(name="deliveryDate", type=DateType)
orderDate_prop = Property(name="orderDate", type=DateType)
basemodel_class.attributes = {deliveryDate_prop, orderDate_prop}
...
POLICY_CONSTRAINT = Constraint(
    ...
    expression='context BaseModel inv: self.deliveryDate %3E%3D self.orderDate %2B 90',
    language="OCL"
)
...
dynamic_obj.deliveryDate = value
print(f"Set deliveryDate = {value} (from test description)")

value = extract_value_from_text(test_description, "DateType")
dynamic_obj.orderDate = value
print(f"Set orderDate = {value} (from test description)")

context_om = ObjectModel(name="BaseModelModel", objects={dynamic_obj})


# --- Evaluate Policy Constraint ---
print(f"\nTesting Policy #1: 'minimum delivery date for a software is 90 days'")
...
if EVAL_MODE == "OCL":
    ...
    expr = "self.deliveryDate %3E%3D self.orderDate %2B 90".strip()
